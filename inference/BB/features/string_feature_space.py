...
from PBKB.inference.BB.utils import get_n_gram_to_index
from PBKB.inference.BB.exceptions import InvalidNGramError


def build_feature_space_for_each_n_gram_length(alphabet, Y, min_n, max_n, is_using_position):
    feature_spaces = {}
    for n in range(min_n, max_n + 1):
        if is_using_position:
            feature_spaces[n] = build_feature_space_with_positions(alphabet, Y, n)
        else:
            feature_spaces[n] = build_feature_space_without_positions(alphabet, Y, n)
    return feature_spaces


def build_feature_space_without_positions(alphabet, Y, n):
...
def build_feature_space_with_positions(alphabet, Y, n):
